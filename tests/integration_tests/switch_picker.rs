...
#[rstest]
fn test_switch_picker_print_outputs_branch_without_switching(mut repo: TestRepo) {
    repo.remove_fixture_worktrees();
    repo.run_git(&["remote", "remove", "origin"]);
    ...
    // Run `wt switch --print`, filter to "target", press Enter to select
    let result = exec_in_pty_with_input_expectations(
        wt_bin().to_str().unwrap(),
        &["switch", "--print"],
        repo.root_path(),
    ...
    assert_eq!(
        result.exit_code, 0,
        "Expected exit code 0 for --print selection"
    );

    let screen = result.screen();

    // --print should output the branch name
    assert!(
        screen.contains("target-branch"),
        "Expected branch name in output with --print.\nScreen:\n{}",
        screen
    );

    // --print should NOT emit a cd directive (read-only operation)
    let directives = std::fs::read_to_string(&directive_path).unwrap_or_default();
    assert!(
        !directives.contains("cd '"),
        "Directive file should NOT contain cd command with --print, got: {}",
        directives
    );
}
