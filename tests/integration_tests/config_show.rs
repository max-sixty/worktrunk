...
fn test_config_show_json_with_project_config(repo: TestRepo, temp_home: TempDir) {
    ...
    // Identifier is computed from the primary remote (fixture origin url:
    // `../origin_git` — parses as a non-host URL, so we get the raw path with
    // `.git` stripped).
    assert_eq!(json["project"]["identifier"].as_str().unwrap(), "../origin");
}
