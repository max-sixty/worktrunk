...
/// Test fork PR lookup falls back to upstream remote when origin returns 404.
///
/// User scenario (issue #1925):
/// 1. User forked owner/repo to contributor/repo
/// 2. User cloned their fork: origin → contributor/repo
/// 3. User added upstream remote: upstream → owner/repo
/// 4. PR #42 exists on owner/repo, not contributor/repo
/// 5. `wt switch pr:42` should find the PR via the upstream remote
#[rstest]
fn test_switch_pr_fork_fallback_to_upstream(#[from(repo_with_remote)] repo: TestRepo) {
    // Create a PR ref on the remote that can be fetched
    repo.run_git(&["checkout", "-b", "pr-source"]);
    fs::write(repo.root_path().join("pr-file.txt"), "PR content").unwrap();
    repo.run_git(&["add", "pr-file.txt"]);
    ...
    // Push the ref to the bare remote as refs/pull/42/head
    repo.run_git(&["push", "origin", &format!("{}:refs/pull/42/head", sha)]);
    repo.run_git(&["checkout", "main"]);

    // Save bare remote URL before changing origin
    let bare_url = String::from_utf8_lossy(
        &repo
            .git_command()
            .args(["config", "remote.origin.url"])
    ...
    // Set origin to the fork (contributor's repo)
    repo.run_git(&[
        "remote",
        "set-url",
        "origin",
    ...
    // Add upstream remote pointing to the org repo
    repo.run_git(&[
        "remote",
        "add",
        "upstream",
    ...
    // Redirect both GitHub URLs to the local bare remote for actual git operations.
    // Use --add for the second value since url.X.insteadOf is multi-valued.
    repo.run_git(&[
        "config",
        &format!("url.{}.insteadOf", bare_url),
        "https://github.com/contributor/test-repo.git",
    ...
    // Mock gh to return 404 for fork, success for upstream
    let mock_bin = repo.root_path().join("mock-bin");
    fs::create_dir_all(&mock_bin).unwrap();
    copy_mock_binary(&mock_bin, "gh");
    ...
    // Use compound matching: fork path → 404, upstream path → success
    MockConfig::new("gh")
        .version("gh version 2.0.0 (mock)")
        .command(
            "api repos/contributor/test-repo/pulls/42",
            MockResponse::output(r#"{"message":"Not Found","status":"404"}"#)
                .with_stderr("gh: Not Found (HTTP 404)")
                .with_exit_code(1),
        )
        .command(
            "api repos/owner/test-repo/pulls/42",
    ...
    settings.bind(|| {
        let mut cmd = make_snapshot_cmd(&repo, "switch", &["pr:42"], None);
        configure_mock_gh_env(&mut cmd, &mock_bin);
        assert_cmd_snapshot!("switch_pr_fork_fallback_to_upstream", cmd);
    });
}
