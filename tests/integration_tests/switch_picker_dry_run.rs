...
/// An unsortable `[list] sort` key aborts `wt list`, but the picker can't
/// surface an abort mid-render: it stashes a warning and falls back to the
/// default row order. Covers the `progressive_handler` degradation arm the sort
/// spec takes in `collect`.
#[rstest]
fn test_picker_dry_run_invalid_sort_key_degrades(mut repo: TestRepo) {
    repo.add_worktree("feature-a");
    std::fs::write(
        repo.test_config_path(),
        r#"[list]
...
"#,
    ...
    assert!(
        output.status.success(),
        "picker degrades on a bad sort key instead of aborting; stderr:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );

    let stderr = String::from_utf8(output.stderr).expect("stderr is utf-8");
    assert!(
        stderr.contains("Sort order ignored"),
        "expected a stashed degradation warning on stderr, got: {stderr}"
    );
    assert!(
        stderr.contains("not sortable"),
        "the warning carries the reason, got: {stderr}"
    );

    let stdout = String::from_utf8(output.stdout).expect("stdout is valid JSON");
    let parsed: serde_json::Value = serde_json::from_str(&stdout).expect("stdout is valid JSON");
    let rows = parsed["rows"].as_array().expect("top-level `rows` array");
    assert!(
        rows.iter()
            .any(|r| r.as_str().unwrap_or("").contains("feature-a")),
        "rows still render in the default order, got: {stdout}"
    );
}
