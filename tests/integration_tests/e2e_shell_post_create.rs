...
#[case("bash")]
#[case("fish")]
fn test_shell_integration_post_create_background(#[case] shell: &str, repo: TestRepo) {
...
#[rstest]
fn test_bash_shell_integration_post_create_parallel(repo: TestRepo) {
...
#[cfg(unix)]
#[rstest]
fn test_fish_shell_integration_post_create_background(repo: TestRepo) {
    // Create project config with background command
    let config_dir = repo.root_path().join(".config");
    fs::create_dir_all(&config_dir).unwrap();
    fs::write(
