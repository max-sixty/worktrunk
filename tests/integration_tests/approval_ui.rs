...
fn test_approval_single_command(repo: TestRepo) {
    repo.write_project_config(r#"pre-create = "echo 'Worktree path: {{ worktree_path }}'""#);
...
fn test_yes_flag_does_not_save_approvals(repo: TestRepo) {
    repo.write_project_config(r#"pre-create = "echo 'test command' > output.txt""#);
...
fn test_already_approved_commands_skip_prompt(repo: TestRepo) {
    ...
    repo.write_project_config(r#"pre-create = "echo 'approved' > output.txt""#);
