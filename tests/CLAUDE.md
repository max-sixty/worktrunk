...
Of the binaries the suite spawns, only `mock-stub` can be missing or stale, because it lives in a separate workspace package: under `cargo test`, a target filter (`--lib`, `--test integration`, …) deselects the dummy test that builds it, so the run gets whatever stub sits in `target/` — a fresh tree panics with "mock-stub binary not found", and a warm one runs a stale stub whose failures reflect the code it was built from. Fix: `cargo build -p mock-stub`, or use `cargo nextest run` / `cargo llvm-cov nextest`, whose `build-bins` setup script rebuilds the stub on any run that selects the integration binary, target-filtered or not. The `wt` binary the tests spawn has no such gap: cargo rebuilds a package's own binaries whenever its integration tests build, under every runner and filter, and the `CARGO_BIN_EXE_wt` the harness resolves at runtime names that just-built binary; outside a cargo runner the suite panics ("CARGO_BIN_EXE_wt not set") rather than guessing a path. `cargo build --bin wt` recompiling right after a test run is the bin-only build being a separate cached unit (a different feature graph), not evidence the tests ran stale code.
