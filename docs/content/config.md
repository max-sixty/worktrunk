...
- `{{ commit_details }}` — <span class="badge-experimental"></span> list of commits being squashed; each renders as its subject and exposes `.subject` / `.body`
- `{{ commits }}` — list of commit subjects being squashed (deprecated in favor of `commit_details`; `wt config update` rewrites it)
