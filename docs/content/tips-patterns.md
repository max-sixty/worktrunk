...
Custom emoji markers show agent state in `wt list`. The [Codex](@/codex.md), [Claude Code](@/claude-code.md), and [OpenCode plugin](https://github.com/max-sixty/worktrunk/tree/main/dev/opencode-plugin.ts) integrations can set these automatically when their hooks are active:
