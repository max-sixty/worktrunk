...
**`{% raw %}` around documented template syntax:** Zola 0.23 renders every `.md` file through Tera, so a documented minijinja example like `{{ branch | hash_port }}` is otherwise read as a Zola expression and fails the build. Zola's only opt-outs are `{% raw %}` and the whole-file `skip_content_templating` glob, and skipping a file would take its `terminal` calls with it, so the pages that document template syntax are the same pages that need templating on. `wrap_template_syntax()` (`src/docs.rs`) therefore wraps *between* component calls: one region per run of content between two calls, markers riding on the first and last line rather than on lines of their own, which keeps a page to a handful of markers and can't split a paragraph or a list. `strip_template_syntax_wrappers()` inverts what wrap emits — a documented `{% raw %}` / `{% endraw %}` inside a region travels as an escape sequence, so a published page round-trips whatever it says about those tags. Text that hasn't been through wrap has no such protection: a bare marker opening or closing a line is indistinguishable from a region marker and gets blanked on read, and the re-wrapped result is a fixed point, so keep documenting those tags mid-line (as `extending.md` and `faq.md` do), never alone on a line. The sync test strips on read and wraps on write (`read_docs_page` / `write_docs_page`), so `docs/content/*.md` is authored and compared without the wrappers and published with them — don't add them by hand.
