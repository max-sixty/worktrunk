...
**`{% raw %}` around documented template syntax:** Zola 0.23 renders every `.md` file through Tera, so a documented minijinja example like `{{ branch | hash_port }}` is otherwise read as a Zola expression and fails the build. `wrap_template_syntax()` (`src/docs.rs`) wraps the fenced blocks and lines that carry template syntax; `strip_template_syntax_wrappers()` recovers the source form. The sync test strips on read and wraps on write (`read_docs_page` / `write_docs_page`), so `docs/content/*.md` is authored and compared without the wrappers and published with them — don't add them by hand. Strip recognises only the wrapper forms wrap emits, and a `{% raw %}` or `{% endraw %}` alone on a line reads as one of them whether this module emitted it or the page documents it: such a line is dropped, and the result is a fixed point of wrap, so the sync test won't flag the loss. Document those tags mid-line (as `extending.md` and `faq.md` do), never alone on a line.
