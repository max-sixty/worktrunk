...
The patch-id check only diffs default-branch commits that touch the branch's files, and caps how many so a single check stays fast; a squash merge whose files were changed again in hundreds of commits since falls outside the cap and needs `-D` to remove.
