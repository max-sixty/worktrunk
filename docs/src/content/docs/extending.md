...
Hooks and aliases live in the same TOML config and share the [template engine](/hook/#template-variables). User config is trusted; project config requires approval on first run. When both define the same name, both run (user first).

## Hooks

Ten hooks cover five lifecycle events — switch, start, commit, merge, remove — each with a blocking `pre-` variant (failure aborts the operation) and a background `post-` variant. [`wt hook`](/hook/#hook-types) maps each hook to its timing and typical uses.
...
See [`wt hook`](/hook/) for the full reference and built-in recipes (dev server per worktree, database per worktree, progressive validation). [Tips & Patterns](/tips-patterns/) has more.
...
Aliases use the same [template engine as hooks](/hook/#template-variables): variables, [filters](/hook/#worktrunk-filters), [functions](/hook/#worktrunk-functions), and [`--KEY=VALUE` smart routing](/hook/#passing-values) (bind if the template references `KEY`, else forward to `{{ args }}`). For example, `wt deploy --env=staging` sets `{{ env }}`.
...
For indexing (`{{ args[0] }}`), looping, and counting, see [Passing values](/hook/#passing-values).
...
`[[aliases.NAME]]` defines a pipeline using the [same `[[block]]` semantics as hooks](/hook/#hook-forms): blocks run in order, keys within a block run concurrently, and a step failure aborts the remainder.
