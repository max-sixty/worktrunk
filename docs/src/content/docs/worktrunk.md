...
<figure class="demo">
<picture>
  <source srcset="/assets/docs/dark/wt-core-mobile.gif" media="(prefers-color-scheme: dark) and (max-width: 42rem)" data-demo-theme="dark" data-demo-media="(max-width: 42rem)">
  <source srcset="/assets/docs/light/wt-core-mobile.gif" media="(max-width: 42rem)" data-demo-theme="light" data-demo-media="(max-width: 42rem)">
  <source srcset="/assets/docs/dark/wt-core.gif" media="(prefers-color-scheme: dark)" data-demo-theme="dark">
...
> Free code signing provided by [SignPath.io](https://signpath.io/), certificate by [SignPath Foundation](https://signpath.org/) — [policy](/code-signing/).
...
This creates a new branch and worktree, then switches to it. Do your work, then check all worktrees with [`wt list`](/list/):
...
The `-x` flag runs a command after switching; arguments after `--` are passed to it. Configure [post-start hooks](/hook/#hook-types) to automate setup (install deps, start dev servers).

## Next steps

- Learn the core commands: [`wt switch`](/switch/), [`wt list`](/list/), [`wt merge`](/merge/), [`wt remove`](/remove/)
- Set up [hooks](/hook/) for automated setup
- Explore [LLM commit messages](/llm-commits/), [interactive
  picker](/switch/#interactive-picker), [Claude Code integration](/claude-code/), [CI
  status & PR links](/list/#ci-status)
- Browse [tips & patterns](/tips-patterns/) for recipes: aliases, dev servers, databases, agent handoffs, and more
- [Extending Worktrunk](/extending/) — customize workflows with hooks & aliases
