...
- **`wt config shell install` no longer writes the zsh integration line into a `.zshrc` in the current directory**: an exported-but-empty `ZDOTDIR` was taken at face value, so zsh's config path collapsed to the relative `.zshrc` — landing the line in a dotfiles checkout, say, while `~/.zshrc` went untouched and `wt config show` reported from the same wrong file. Uninstall rewrites rc files whole, so the same resolution decided which file that rewrite hit. `$ZDOTDIR` now has to be an absolute path to be honoured, the guard worktrunk already applied to `$XDG_DATA_HOME` and `$XDG_CONFIG_HOME`; anything else falls back to `$HOME`, which is also where `wt config show` reads back from. A relative value such as `ZDOTDIR=dotfiles` was the same bug and is covered by the same guard — zsh resolves one against its own startup directory, which `wt` cannot know at install time. The Nushell uninstall's search for wrappers stranded by older versions goes through the same check.
