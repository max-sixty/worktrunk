...
- **Config migrations reach a `[projects]` scope written inline**: the scope walk behind the deprecated-key rules visited only standard tables, so `"host/org/repo" = { merge = { no-ff = true } }` was skipped while the equivalent `[projects."host/org/repo".merge]` section migrated. `UserProjectOverrides` has no alias for the old key, so the unmigrated setting was dropped from the typed config on load and the per-project non-fast-forward merge silently stopped applying. Both the `projects` container and each entry inside it are now migrated in either shape, and an entry no rule touched keeps its formatting. This covers every rule on that walk: `merge.no-ff`, `switch.no-cd`, the removed `list.task-timeout-ms`, and the `[select]`/`[ci]`/`[commit-generation]` section renames.
