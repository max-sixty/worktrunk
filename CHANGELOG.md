...
- **Worktree safety checks no longer inherit Git status display filters**: `status.showUntrackedFiles = no` hid untracked files, while `submodule.<name>.ignore = all` hid dirty submodules, so `wt remove`, `wt merge`, or unattended `wt step prune` could discard work without `--force`. Safety checks now force both kinds of changes visible and name the path that blocks removal. Auto-staging warnings for `wt merge`, `wt step commit`, `wt step squash`, and `wt step relocate --commit` likewise override hidden-untracked settings, report the full count, and preview up to 10 terminal-safe paths without terminal-control, bidirectional-text, or non-UTF-8 ambiguity. `wt merge --stage=tracked` considers tracked worktree changes, while `--stage=none` considers only the existing index; both leave untracked files alone when retaining the worktree, refuse residual changes before updating the target when cleanup will run, and detect staged submodule updates hidden by `diff.ignoreSubmodules`. Merge cleanup also rechecks immediately before the target ref update, so dirty submodules or changes created during approval cannot leave a partially updated merge. A `pre-remove` hook can deliberately preserve a worktree by locking it; `wt remove`, merge cleanup, prune, and the picker now all treat that post-planning lock as a successful skip without running post-removal hooks. Lock preservation also skips `--reap`, renders as a successful picker outcome, and `wt remove --format=json` distinguishes `removed`, `deferred`, and `preserved_locked` worktrees.
