...
- **An exported-but-empty or relative `$XDG_CONFIG_HOME` no longer sends worktrunk looking for other tools' config beside the directory it was run from**: the value was taken at face value, so Gitea detection looked for a relative `tea/config.yml` — the user's real `~/.config/tea/config.yml` never read, and, where the repository happened to contain a `tea/config.yml`, that file consulted instead. With no logins found, `wt list --full` dropped the CI column for Gitea repositories and `wt switch pr:<n>` chose GitHub for a self-hosted host tea was logged in to. `wt config plugins opencode install` took a relative value the same way, writing the plugin under the invocation directory, and the Nushell uninstall's search for wrappers stranded by older versions did too. The reads that mean "the XDG config directory" now delegate to `etcetera`, which worktrunk already uses to resolve its own config, so they get the spec's own exclusions — an unset *or empty* value means `~/.config`, and a relative path is invalid and ignored. The fourth read follows Nushell's rule rather than the spec's, and now delegates to `nu-config`, Nushell's own path-resolution crate, instead of restating that rule: where `nu` isn't on `PATH` to be asked, the wrapper's location is Nushell's own answer for it and can't drift from it. It resolves to the same directory the copied rule did, and additionally honors `$NU_VENDOR_AUTOLOAD_DIR`.
