...
- **An alias that binds `dry_run` can take `--dry-run` again**: `wt <alias> --dry-run <value>` answered the retired-flag error even when the alias's own template referenced `{{ dry_run }}`, while `--dry-run=<value>` bound normally — so one flag behaved two ways. The error now fires only for an alias that doesn't reference the name, matching how `--help` and `--yes` already yield to a template that binds them. ([#4058](https://github.com/max-sixty/worktrunk/pull/4058))
