...
- **`wt config show` and `wt config update` report a migration diff git could not render**: the preview read only `git diff --no-index`'s stdout, so a git that failed before writing a patch — a broken `diff.external` program, an unparsable `diff.*` value — was indistinguishable from two identical files, and the `Proposed diff:` block simply vanished from the output and from the preview `wt config update` prompts on. The exit status now decides which of the three it was, a failure prints git's own stderr under a warning, and the patch worktrunk renders for itself passes `--no-ext-diff` so a configured external diff program is never handed its temp files. The migration is computed in memory and was never affected, so only the preview ever degraded. ([#4126](https://github.com/max-sixty/worktrunk/pull/4126))
