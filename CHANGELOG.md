...
- **Saving user config no longer drops unknown keys written inside an inline section**: `save_to` preserves schema-unknown keys — a typo, or a field from a newer `wt` — at every nesting level, but only the standard-table branch of its merge applied that preservation tree. A section written inline, `merge = { squash = false, future-option = true }`, was replaced wholesale whenever its contents differed from the serialized form, which an unknown key guarantees, so an older `wt` deleted `future-option` during a save triggered by an unrelated setting. The inline branch now merges through the same recursive path as a standard table, and an inline section nothing changed keeps its formatting.

- **An inline config section that worktrunk rewrites as a standard table no longer produces a config file `wt` can't read**: the rewrite carried the key's own decor — the comment above it, the blank line before it, the space before `=` — into the table header, producing `[# why squash is off\nmerge ]`, and every later command failed with a TOML parse error pointing at a line the user never wrote. Two paths converted that way: saving a changed value inside `merge = { squash = false }`, and the deprecation migration converting an inline `commit` or `switch` parent so `[commit.generation]` or `[switch.picker]` could be added under it. The second runs before every load, so a `commit-generation` or `[select]` section under an inline parent that had a comment or a blank line above it stopped the config from loading at all. Leading comments and blank lines now sit above the header where they belong.
