...
- **Changing a value inside an inline config section no longer writes a config file `wt` can't read back**: rewriting `merge = { squash = false }` as a standard table carried the key's own decor — the comment above it, the blank line before it, the space before `=` — into the table header, producing `[# why squash is off\nmerge ]`. Every later command then failed with a TOML parse error, and the user's own comment was what broke it. The leading comments and blank lines now sit above the header where they belong.
