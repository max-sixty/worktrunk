...
- **Gitea detection reads tea's config from `~/.config` when `$XDG_CONFIG_HOME` is exported but empty**: the value was taken at face value, so the lookup became the relative `tea/config.yml` and resolved against whatever directory `wt` was invoked from — the user's real `~/.config/tea/config.yml` never read, and, where the repository happened to contain a `tea/config.yml`, that file consulted instead. With no logins found, `wt list --full` dropped the CI column for Gitea repositories and `wt switch pr:<n>` chose GitHub for a self-hosted host tea was logged in to. An empty value now means the `$HOME/.config` default and a relative one is ignored, as the XDG base directory spec requires and as the Nushell wrapper's `$XDG_DATA_HOME` lookup already did.
