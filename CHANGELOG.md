...
- **`wt config update` no longer changes what a template renders when renaming a retired variable**: it rewrote `{{ repo_root }}` inside a quoted string, and a name the template rebinds with `{% set %}`. A template that binds a retired name is now left unmigrated and unwarned, and fails to render until edited. ([#4124](https://github.com/max-sixty/worktrunk/pull/4124), thanks @zach-hammad-vs for reporting)
