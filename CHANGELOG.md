...
- **Cached integration answers from before 0.79.0 can still be wrong**: the check keys on the branch and target tips with no expiry, so an answer an earlier version computed under `diff.relative` or `submodule.<name>.ignore` is still on disk. Run `wt config state cache clear` in each repository where either applies — `submodule.<name>.ignore` counts whether it comes from your git config or the repository's `.gitmodules`. ([#4146](https://github.com/max-sixty/worktrunk/pull/4146), [#4147](https://github.com/max-sixty/worktrunk/pull/4147))
