...
- **Concurrent commands in alias table form**: Aliases defined with multi-entry table form (`[[aliases.deploy]]`) now run their commands concurrently; pipeline form (`commands = [...]`) remains serial. Output streams through a shared executor that prefixes every line with a colored `{label} │ `, identifying which child produced it. Concurrent writes no longer interleave mid-line or drop lines under load. ([#2089](https://github.com/max-sixty/worktrunk/pull/2089), [#2145](https://github.com/max-sixty/worktrunk/pull/2145))
