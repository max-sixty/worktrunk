...
- **`pre-*` hook table form temporarily deprecated ahead of concurrent-execution unification**: Today, `pre-*` hooks can only run serially: pipeline form is serial by construction, and table form (`[[pre-merge]]`) — which already runs concurrently for `post-*` hooks and for aliases — is forced serial for `pre-*`. A future release will lift that restriction so `pre-*` table form runs concurrently too, matching `post-*` and aliases. To make the switch safe, `pre-*` table form is deprecated now: at load time it's rewritten to pipeline form in memory so current serial behavior is preserved, but the on-disk config isn't touched — run `wt config update` to write the migration, or rewrite by hand if you want to keep table form and opt into the upcoming concurrent semantics when they ship. ([#2135](https://github.com/max-sixty/worktrunk/pull/2135), [#2151](https://github.com/max-sixty/worktrunk/pull/2151))

- **`--KEY=VALUE` shorthand for alias and hook variables**: `wt step deploy --env=staging` and `wt hook pre-start --branch=feature/test` now work without the `--var` prefix. `--my-var=value` becomes `{{ my_var }}` in templates. Hooks also accept **custom variable names** (previously restricted to a known list, matching alias behavior) and warn when a `--var` isn't referenced by any template — catching typos like `--branch=feature`. ([#2091](https://github.com/max-sixty/worktrunk/pull/2091), [#2096](https://github.com/max-sixty/worktrunk/pull/2096), [#2117](https://github.com/max-sixty/worktrunk/pull/2117))

- **`wt step` discovers configured aliases**: Running `wt step` (or `wt step --help`) now lists merged user+project aliases alongside the built-in subcommands, each with a one-line template summary. Aliases that shadow a built-in are flagged `(shadowed by built-in)`. ([#2131](https://github.com/max-sixty/worktrunk/pull/2131), [#2141](https://github.com/max-sixty/worktrunk/pull/2141))

- **Shell completions for external `wt-*` subcommands**: Tab completion now discovers `wt-*` binaries on PATH and forwards completion requests to them, so `wt sync --<tab>` surfaces the external command's flags. Builds on the git-style external subcommand dispatch in 0.36.0. ([#2074](https://github.com/max-sixty/worktrunk/pull/2074), thanks @pablospe)

- **Persistent on-disk cache for expensive git operations**: Five SHA-keyed probes that previously ran live on every `wt list` and `wt switch` — merge-tree conflict checks, the integration/add-probe, `is-ancestor`, `has-added-changes`, and branch diff stats — are now cached to disk under `.git/wt/cache/`. Because commit SHAs are content-addressed, cached results never go stale; an LRU bound (5000 entries per kind) keeps disk usage bounded. Several user-visible wins flow from this:

    - **`wt list` and the `wt switch` picker open much faster on big repos**, especially those with many stale branches. Warm cache means repeated invocations skip the expensive probes entirely; cold cache still benefits from the next item.
    ...
    - **The picker now shows the same status info as `wt list`.** The old "skip stale branches" shortcut hid conflict and ahead/behind info on branches 50+ commits behind main to keep the picker responsive — the cache makes the shortcut unnecessary, so stale branches now display full status. Dead plumbing (`EXPENSIVE_TASKS`, `stale_branches` skip flag, env-var gates) removed.
...
- **Concurrent commands in alias table form**: Aliases defined with multi-entry table form (`[[aliases.deploy]]`) now run their commands concurrently, where pipeline form (`commands = [...]`) remains explicitly serial. Each child's output streams through a shared executor that prefixes every line with a colored `{label} │ ` so you can tell which child said what, and concurrent writes no longer interleave mid-line or drop lines under load. ([#2089](https://github.com/max-sixty/worktrunk/pull/2089), [#2145](https://github.com/max-sixty/worktrunk/pull/2145))

- **Pipeline structure in alias announcements**: Aliases now announce as `Running alias deploy: install; build, lint` showing pipeline structure, instead of the bare alias name. ([#2092](https://github.com/max-sixty/worktrunk/pull/2092))
