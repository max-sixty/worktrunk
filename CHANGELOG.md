...
- **Template-variable migration no longer mis-reads a tag that contains a quoted delimiter**: the rewriter found a tag's end with a plain search for the closing `}}`, while MiniJinja — which decides which variables are in scope — tokenizes strings first. In `{{ "}} " ~ repo_root }}` the tag was cut at the quoted delimiter, so the real `repo_root` after it was never migrated; where a second string held a literal `{{ repo_root }}`, that output text was rewritten instead of the reference beside it. The scan now skips quoted strings, in `{{ }}` and `{% %}` alike. A block tag's keyword is read the same way MiniJinja reads it, `+` whitespace control included, so `{%+ raw %}` is recognized as a raw block and its literal contents are left alone.

- **Template-variable migration leaves a deprecated name that a block binds alone**: `{{ repo_root }}{% set repo_root = "local" %}{{ repo_root }}` reads the global once and the local once, but the rewriter has no scope tracking and renamed both, so the last expression silently pointed at a different value. A name bound by `set`, `for`, or `with` anywhere in the template is now left unmigrated — and stops warning, since the warning fires exactly when an update would rewrite the file. Targets are read the way MiniJinja's own `parse_assignment` reads them, so a nested parenthesized tuple — `{% for (repo_root, x) in items %}` — binds as a bare name does. Binding the *canonical* name holds the rename back for the same reason from the other side: `{% for repo_path in items %}` around a `{{ repo_root }}` would walk a genuine global read into the loop variable. A name only *used* inside a block tag, `{% if repo_root %}`, still migrates, as does one whose only binding sits inside `{% raw %}`, where MiniJinja binds nothing.
