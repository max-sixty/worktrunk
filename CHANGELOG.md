...
- **Faster shell-command dispatch (~10ms per invocation)**: `Cmd::stream`'s `forward_signals=true` path was running a `try_wait` + `thread::sleep(10ms)` + `signals.pending()` loop, imposing a ~13ms wall floor on every alias and hook subprocess dispatch — ~70% of `execute_shell_command` time for short aliases. Replaced with an event-driven shape: a listener thread blocks on `Signals::forever()` and forwards SIGINT/SIGTERM; the main thread blocks on `child.wait()`. `wt step <trivial-alias>` 30.4ms → 20.4ms via hyperfine (1.49× faster). The matching 25ms poll in `output/concurrent.rs::spawn_signal_forwarder` got the same treatment, dropping Ctrl-C latency from "up to 25 ms after the press" to "as soon as signal-hook delivers". ([#2537](https://github.com/max-sixty/worktrunk/pull/2537), [#2538](https://github.com/max-sixty/worktrunk/pull/2538))

- **Faster cold-start prewarm**: A plain `wt <alias>` invocation used to spawn two `git rev-parse` forks before alias dispatch (`--git-common-dir` from `init_command_log`, then the `prewarm_info` batch from `project_config_path`). One merged `git rev-parse` fork now populates both, with downstream callers hitting memory. ([#2541](https://github.com/max-sixty/worktrunk/pull/2541))

- **Parallel user + project config load**: `LoadedConfigs::load(&repo)` loads user config and project config on scoped threads alongside a `project_identifier` cache warm-up. On a cold cache the wall-clock cost is the longest pole (~5ms) instead of the sum (~13ms sequential); `try_alias` total drops from ~33ms to ~26ms. ([#2543](https://github.com/max-sixty/worktrunk/pull/2543))
...
- **`wt list`, `wt remove`, `wt merge`, and `wt step prune` integration detection in the local/upstream-diverged case**: `Repository::integration_reason` picked a single target via `effective_integration_target` and checked only that, silently dropping half of history when local `main` and `origin/main` had diverged. After `wt merge` advanced local `main` (still diverged from `origin/main`), the upstream-only check would report the just-merged branch as unmerged and skip removal. The symmetric hole existed for `wt remove` and rendered as wrong status in the `wt list` integration column. `integration_reason` now ORs over local and upstream — integrated if either matches — and `wt step prune`/`wt list` route through it instead of `effective_integration_target` (which is now deleted). ([#2507](https://github.com/max-sixty/worktrunk/pull/2507), [#2513](https://github.com/max-sixty/worktrunk/pull/2513), [#2515](https://github.com/max-sixty/worktrunk/pull/2515))

- **`wt step copy-ignored` no longer writes outside the destination worktree through symlinked subdirectories**: A linked worktree containing a symlinked destination directory like `target -> /tmp/outside` would let `copy-ignored` create nested directories and files through the symlink, writing outside the destination worktree; `--force` made the same boundary issue more risky because it could overwrite outside files. New guarded copy entry points resolve the destination ancestry with `canonicalize_with_parents` and reject paths whose resolved parent chain escapes the destination root. Leaf-symlink behavior is unchanged: without `--force` leaf symlinks are skipped; with `--force` the symlink itself is replaced. ([#2501](https://github.com/max-sixty/worktrunk/pull/2501), thanks @douglas)

- **`wt step copy-ignored` no longer hardcodes `.pi/` in built-in excludes**: The default excludes list shipped with `.pi/` baked in alongside `.git/`/`.DS_Store`/etc. — a sledgehammer fix from an unrelated debugging session that has no place as a project-agnostic default. Removed; users who need it can add it via the `[step.copy-ignored]` config. ([#2527](https://github.com/max-sixty/worktrunk/pull/2527), thanks @indigoviolet for reporting [#2526](https://github.com/max-sixty/worktrunk/issues/2526))

- **`wt step for-each -- <argv>` preserves quoting and argument boundaries**: The post-`--` argv was rebuilt with `args.join(" ")` and passed through `sh -c`, dropping the user's quoting and argv boundaries — anything with spaces, `;`, or shell metacharacters inside an argv element broke. The post-`--` argv is now exec'd directly via `Cmd::new(program).args(rest)`, with no implicit shell wrapper. Users wanting shell features (pipes, redirects, `$VAR`, globs) write `sh -c '<snippet>'` explicitly — same pattern as `xargs`, `find -exec`, `kubectl exec --`, and `docker run`. (Breaking: previously `wt step for-each -- <snippet>` accepted shell snippets without `sh -c`; `for-each` is `[experimental]`, so the small breakage is acceptable.) ([#2465](https://github.com/max-sixty/worktrunk/pull/2465))
...
- **`RefSnapshot` replaces ambient ref-keyed caches**: `RepoCache` cached ref-name → SHA in `commit_shas: DashMap<String, String>`, plus composite caches (`integration_reasons`, `effective_integration_targets`, `ahead_behind`, `integration_target` OnceCell, `tree_shas`, `resolved_refs`). When wt itself moved a ref mid-command — `wt merge` running `git update-ref refs/heads/<target>` to advance the local target — those caches went stale, and downstream reads returned pre-write SHAs. Callers now explicitly capture a `RefSnapshot` at known points and thread it through read paths; the `head_shas` per-worktree cache is also dropped (`WorkingTree::head_sha` reads fresh) so `{{ commit }}` reflects post-rebase HEAD movement in subsequent hooks. (Breaking library API: `Repository::ahead_behind`, `batch_ahead_behind`, `effective_integration_target`, `integration_target` removed; `Repository::integration_reason` takes 3 parameters; `worktrunk::copy::copy_dir_recursive` and `copy_leaf` gained a parameter; `worktrunk::git::compute_integration_lazy`, `remove_worktree_with_cleanup`, `delete_branch_if_safe` gained parameters.) ([#2528](https://github.com/max-sixty/worktrunk/pull/2528), [#2530](https://github.com/max-sixty/worktrunk/pull/2530))

- **`refactor(alias)!` source-tag steps and decide EXEC per-step**: Same-name user+project alias collisions used to scrub EXEC for the whole merged pipeline — a conservative side effect of name-keyed merging in `append_aliases`. With per-step decisions, the user's own steps keep the EXEC relaxation; only the project-authored steps scrub. Strictly more permissive than the prior approach. Internally, `AliasMap = BTreeMap<String, AliasEntry { user, project }>` replaces the merged `BTreeMap<String, CommandConfig>`, and a single `sourced_steps_to_foreground(steps, &PipelineKind)` resolver decides EXEC per source tag. Hook-only metadata (`hook_type`, `display_path`) lifts onto `PipelineKind::Hook`, removing four `.expect("hook pipelines always set hook_type")` sites. ([#2474](https://github.com/max-sixty/worktrunk/pull/2474), [#2521](https://github.com/max-sixty/worktrunk/pull/2521))

- **`Span` RAII guard for in-process `[wt-trace]` attribution**: Subprocess records (`cmd="git …"`) accounted for child-process work; everything else — config load, repo open, template render, fork+exec wait — was opaque. New `worktrunk::trace::Span` records `[wt-trace] span="name" dur_us=…` on drop, parsed by the existing chrome-trace renderer as `ph: "X"` Complete events with `cat: "wt"`. Spans wired through alias dispatch, config load, `build_hook_context` (with per-var sub-spans), `template_render`, and `execute_shell_command`. Open the resulting `trace.json` in https://ui.perfetto.dev — subprocess records (`cat=git`) interleave with span records (`cat=wt`) so cold-cache attribution down to ~µs is straightforward. ([#2539](https://github.com/max-sixty/worktrunk/pull/2539))

- **Codecov uploads enabled on fork PRs**: Drops the `github.repository_owner == 'max-sixty'` guards on both Codecov upload steps in `ci.yaml`. The action falls back to tokenless when `secrets.CODECOV_TOKEN` is empty (which is the case on fork PRs); the major fork-tokenless bug ([codecov-action#1842](https://github.com/codecov/codecov-action/issues/1842)) was fixed in late October 2025. `fail_ci_if_error` on the coverage upload is gated to the main repo so a Codecov flake on a fork PR can't red-X the build. ([#2535](https://github.com/max-sixty/worktrunk/pull/2535))

- **Nightly benchmark results append to a public gist**: Nightly job uploaded `target/criterion` as a 90-day artifact and nothing else; results weren't queryable across runs. New `criterion-to-jsonl.py` walks `target/criterion/**/new/estimates.json` and emits one JSON line per benchmark group (timestamp, commit SHA, mean_ns, stddev_ns); the workflow appends rows to the worktrunk-bot gist's `results.jsonl`. ([#2531](https://github.com/max-sixty/worktrunk/pull/2531), [#2532](https://github.com/max-sixty/worktrunk/pull/2532))
