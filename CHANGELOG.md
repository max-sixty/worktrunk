...
- **Template-variable migration no longer mis-reads a tag that contains a quoted delimiter**: the rewriter found a tag's end with a plain search for the closing `}}`, while MiniJinja — which decides which variables are in scope — tokenizes strings first. In `{{ "}} " ~ repo_root }}` the tag was cut at the quoted delimiter, so the real `repo_root` after it was never migrated; where a second string held a literal `{{ repo_root }}`, that output text was rewritten instead of the reference beside it. The scan now skips quoted strings, in `{{ }}` and `{% %}` alike.

- **Template-variable migration leaves a deprecated name that a block binds alone**: `{{ repo_root }}{% set repo_root = "local" %}{{ repo_root }}` reads the global once and the local once, but the rewriter has no scope tracking and renamed both, so the last expression silently pointed at a different value. A name bound by `set`, `for`, `with`, `macro`, or `call` anywhere in the template is now left unmigrated — and stops warning, since the warning fires exactly when an update would rewrite the file. A name only *used* inside a block tag, `{% if repo_root %}`, still migrates.
