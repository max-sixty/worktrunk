...
- **A valid empty per-project section no longer warns as an unknown field**: `[projects."host/org/repo".list]` is accepted on load, but a section equal to its default serializes away, and the round-trip the unknown-field check compares against only had its *top-level* keys restored. Every command then printed `unknown field projects.host/org/repo.list` for configuration worktrunk had just read correctly. The same restore now reaches inside each `[projects."<id>"]` entry. Genuinely unknown keys there still warn, including one nested inside a known section and a user setting with no per-project form. One behavior rides along: an empty per-project section is now dropped when worktrunk rewrites the config file, as an empty top-level section already was — it was previously kept only because it read as unknown.

- **Rewriting the config no longer strips a forward-compat key that is the only thing in its section**: `save_to` preserves schema-unknown keys so an older worktrunk doesn't silently drop config a newer one wrote, but a section equal to its default serializes away, so the stale-key sweep saw `[list]` — or `[projects."<id>".list]` — as absent from the desired document and removed it along with the `future-knob = 1` inside. A section is now kept whenever unknown keys are recorded below it. An empty one, holding nothing to preserve, is still removed. The same sweep was deleting deprecated keys the same way: `[list] task-timeout-ms` is stripped from the in-memory config on every load and left behind in the file, so writing an unrelated setting removed the emptied `[list]` and the deprecated key with it, silently, and the warning pointing at `wt config update` stopped firing. It now survives until that command migrates it.
