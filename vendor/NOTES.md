...
- **`width_cjk` vs `width` mismatch in the overflow check** — skim's `draw_item` (`skim-0.20.5/src/selection.rs:550`) measures lines with `width_cjk()` (East-Asian-ambiguous chars like `…`, `↑`, `↓` count as 2 columns) when deciding whether to repaint the trailing two columns as `..`, but `LinePrinter` advances by standard `width()` (1 column) — matching what terminals actually render. Any full-width row containing an ambiguous char fails the check spuriously and loses its last two columns to `..` (visible as `2m.` in the picker's Age column on rows with `↑`/`↓`). `--prs` rows pre-clamp against `StyledLine::width_cjk` in `src/commands/picker/prs.rs` to dodge it; worktree rows from `collect` still hit it. A one-word fix in `draw_item` (`width_cjk` → `width`) removes the false overflow.
