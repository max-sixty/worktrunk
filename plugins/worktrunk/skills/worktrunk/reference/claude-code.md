...
The CI reference (`#3035`) links to its pull request, and a configured [dev server URL](https://worktrunk.dev/list/) shows as its port — `:3000`, dim until something answers on it. Both link via OSC 8. Claude Code re-emits the escapes to the terminal rather than drawing them itself, so the terminal decides: one known to render them (`TERM`, `TERM_PROGRAM`, or `FORCE_HYPERLINK=1`) underlines them, and elsewhere they read as plain text.
