...
| Summary | LLM-generated branch summary; requires `--full` or `summary = true` in `[list.columns]`, plus `[list] summary = true` and [`commit.generation`](https://worktrunk.dev/config/#commit) [experimental] |
| Remote⇅ | Commits ahead/behind tracking branch (`remote-commits`) |
| CI | PR/MR number colored by pipeline status; `--full` or `ci = true` |
...
Reuses the [`commit.generation`](https://worktrunk.dev/config/#commit) command — the same LLM that generates commit messages. Enable generation with `summary = true` in `[list]` config and show the column with `--full` or `summary = true` in `[list.columns]`. Results are cached until the branch's diff changes.
