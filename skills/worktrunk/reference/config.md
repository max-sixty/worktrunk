...
Once detected, the result is cached in `worktrunk.default-branch` for fast access. Similarly, `worktrunk.worktree-path` can be set in git config to override the worktree path template for a specific clone — see [Scope options](https://worktrunk.dev/config/#scope-options).
