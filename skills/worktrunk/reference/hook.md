...
Some variables are conditional: `upstream` requires remote tracking; `base`/`target` are only in two-worktree hooks; `pr_number`/`pr_url` are populated for `post-switch`, `pre-start`, and `post-start` hooks when creating via `pr:N` or `mr:N`; `vars` keys may not exist. Undefined variables error — use conditionals or defaults for optional behavior:
