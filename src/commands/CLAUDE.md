...
4. Run `cargo test --test integration test_docs_are_in_sync`. Editing help text also changes the rendered `--help` snapshots, which that test leaves untouched — run `task test-help-snapshots`, review the scoped delta, then `task accept-help-snapshots` (or run the pre-merge hook, which does both).
