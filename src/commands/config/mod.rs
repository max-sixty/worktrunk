...
mod tests {
    ...
    fn test_warn_unknown_keys_user_only_commit_key_redirects_to_user_config() {
        ...
        assert_snapshot!(
            ...
            @r#"[33m▲[39m [33mKey [1mcommit.generation.command[22m belongs in user config (will be ignored); to scope it to this repo, add it under [projects."<id>"] in user config[39m"#);
        assert_snapshot!(
            warn_unknown_keys::<ProjectConfig>("[commit.generation]\ncommand = \"llm\"\n"),
            @r#"[33m▲[39m [33mKey [1mcommit.generation.command[22m belongs in user config (will be ignored); to scope it to this repo, add it under [projects."<id>"] in user config[39m"#);
