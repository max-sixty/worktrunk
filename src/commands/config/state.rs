...
///
/// Matches `.log` (hook output, verbose log), `.md` (diagnostic report),
/// `.jsonl` (command audit log), and `.jsonl.old` (rotated).
...
/// Check if a file is a diagnostic file created by `-vv`.
///
/// These are `verbose.log` (debug log) and `diagnostic.md` (issue report).
    ...
    let (command_log, hook_output, diagnostic): (
        ...
        for entry in &all_entries {
            ...
            if is_command_log_file(&name) {
                ...
                diag.push(to_json(entry));
            } else {
                hook_out.push(to_json(entry));
            }
        }
        (cmd_log, hook_out, diag)
    } else {
        (vec![], vec![], vec![])
    };
