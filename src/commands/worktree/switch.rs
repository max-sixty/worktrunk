...
pub fn run_switch(
    ...
    // prompt and the template pre-flight below agree with what `execute_switch`
    // / `spawn_switch_background_hooks` resolve at run time. Skip entirely when
    // `--no-hooks` / `--no-verify` is in effect: the result is never used.
