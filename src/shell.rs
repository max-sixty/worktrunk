...
/// Result of probing shell integration activation.
///
/// Describes how a command resolves in the user's actual shell environment,
/// after all config files have been sourced.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ShellProbeResult {
    /// Shell function exists (shell integration is working)
    Function,
    /// Alias exists, with the target it points to
    Alias { target: String },
    /// Command found as a binary in PATH (shell function not active)
    Binary { path: String },
    /// Command not found in shell
    NotFound,
    /// Couldn't probe (shell not installed, timeout, error)
    ProbeError { reason: String },
}

impl ShellProbeResult {
    /// Returns true if shell integration would work with this result.
    ///
    /// Only `Function` guarantees shell integration works. `Alias` may or may not
    /// work depending on what it targets.
    pub fn is_function(&self) -> bool {
        matches!(self, ShellProbeResult::Function)
    }

    /// Human-readable description of the probe result.
    pub fn description(&self) -> String {
        match self {
            ShellProbeResult::Function => "shell function".to_string(),
            ShellProbeResult::Alias { target } => format!("alias to `{target}`"),
            ShellProbeResult::Binary { path } => format!("binary at {path}"),
            ShellProbeResult::NotFound => "not found".to_string(),
            ShellProbeResult::ProbeError { reason } => format!("probe failed: {reason}"),
        }
    }
}

/// Probe the shell to determine how a command resolves.
///
/// Spawns an interactive login shell to see how the user's environment
/// resolves the given command name. This catches issues that config-file
/// scanning cannot detect:
/// - Aliases that bypass shell functions
/// - Source order issues
/// - Conditional blocks that don't execute
/// - Shell restart needed after config changes
///
/// # Arguments
/// * `shell` - The shell to probe
/// * `cmd` - The command name to check (e.g., "wt", "git-wt")
///
/// # Returns
/// The result of probing, indicating whether the command is a function,
/// alias, binary, or not found.
///
/// # Example output (bash/zsh)
/// ```text
/// wt is a function                    -> Function
/// wt is aliased to `wt.exe'           -> Alias { target: "wt.exe" }
/// wt is /usr/local/bin/wt             -> Binary { path: "/usr/local/bin/wt" }
/// bash: type: wt: not found           -> NotFound
/// ```
pub fn probe_shell_integration(shell: Shell, cmd: &str) -> ShellProbeResult {
    use std::process::{Command, Stdio};
    use std::time::{Duration, Instant};

    // Build the probe command based on shell type
    let (shell_bin, shell_args, probe_cmd) = match shell {
        Shell::Bash => (
            "bash",
            vec!["-ilc"],
            // Use unique markers to distinguish our output from shell startup noise
            format!(r#"echo __WT_PROBE_START__ && type {cmd} 2>&1 && echo __WT_PROBE_END__"#),
        ),
        Shell::Zsh => (
            "zsh",
            vec!["-ic"],
        ...
        Shell::Fish => (
            "fish",
            vec!["-c"],
            // Fish's type command has different output format
            format!(r#"echo __WT_PROBE_START__; type {cmd} 2>&1; echo __WT_PROBE_END__"#),
        ),
        Shell::PowerShell => {
            // PowerShell uses Get-Command, output format differs
            return probe_powershell(cmd);
        }
    };
    ...
    let mut child = match Command::new(shell_bin)
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        // Prevent subprocess from writing to directive file
        .env_remove(crate::shell_exec::DIRECTIVE_FILE_ENV_VAR)
        // Suppress zsh compinit warnings (same as detect_zsh_compinit)
        .env("ZSH_DISABLE_COMPFIX", "true")
        .spawn()
    {
        Ok(child) => child,
        Err(e) => {
            return ShellProbeResult::ProbeError {
                reason: format!("{shell_bin} not available: {e}"),
            };
        }
    };

    let start = Instant::now();
    let timeout = Duration::from_secs(3); // Slightly longer than compinit probe
    ...
    loop {
        match child.try_wait() {
            Ok(Some(_status)) => {
                ...
                let stdout = String::from_utf8_lossy(&output.stdout);
                return parse_type_output(&stdout, cmd, shell);
            }
            Ok(None) => {
                if start.elapsed() > timeout {
                    let _ = child.kill();
...
/// Parse the output of `type {cmd}` from bash/zsh/fish.
fn parse_type_output(output: &str, cmd: &str, shell: Shell) -> ShellProbeResult {
    // Extract content between our markers to ignore shell startup noise
    let content = if let Some(start) = output.find("__WT_PROBE_START__") {
        let after_start = &output[start + "__WT_PROBE_START__".len()..];
        if let Some(end) = after_start.find("__WT_PROBE_END__") {
    ...
    // Empty or no useful content means not found
    if content.is_empty() {
        return ShellProbeResult::NotFound;
    }

    // Check for "not found" patterns
    if content.contains("not found") || content.contains("Unknown command") {
        return ShellProbeResult::NotFound;
    }

    // Parse based on shell-specific output format
    match shell {
        Shell::Bash | Shell::Zsh => parse_bash_zsh_type(content, cmd),
        Shell::Fish => parse_fish_type(content, cmd),
        Shell::PowerShell => unreachable!("PowerShell uses separate probe"),
    }
}

/// Parse bash/zsh `type` output.
///
/// Examples:
/// - `wt is a function`
/// - `wt is a shell function`
/// - `wt is aliased to `wt.exe'`
/// - `wt is an alias for wt.exe`
/// - `wt is /usr/local/bin/wt`
/// - `wt is a shell builtin`
fn parse_bash_zsh_type(content: &str, cmd: &str) -> ShellProbeResult {
    // Look for the line that describes our command
    for line in content.lines() {
        let line = line.trim();

        // Must start with our command name
        if !line.starts_with(cmd) {
            continue;
        }

        // Function patterns
        if line.contains("is a function") || line.contains("is a shell function") {
            return ShellProbeResult::Function;
        }

        // Alias patterns (bash: `aliased to`, zsh: `is an alias for`)
        if line.contains("aliased to") || line.contains("is an alias") {
            // Extract the target: look for text after the last space or backtick
            let target = extract_alias_target(line);
            return ShellProbeResult::Alias { target };
        }

        // Binary pattern: `wt is /path/to/wt`
        // The path comes after "is "
        if let Some(pos) = line.find(" is ") {
            let after_is = &line[pos + 4..];
            // If it starts with / or contains path separators, it's a binary
            if after_is.starts_with('/') || after_is.contains('\\') {
                return ShellProbeResult::Binary {
                    path: after_is.trim().to_string(),
                };
            }
        }

        // Shell builtin - shouldn't happen for wt but handle it
...
/// Parse fish `type` output.
///
/// Examples:
/// - `wt is a function with definition`
/// - `wt is an alias to git-wt`
/// - `wt is /usr/local/bin/wt`
fn parse_fish_type(content: &str, cmd: &str) -> ShellProbeResult {
    for line in content.lines() {
        let line = line.trim();
        ...
        if line.contains("is a function") {
            return ShellProbeResult::Function;
        }

        if line.contains("is an alias") {
            let target = extract_alias_target(line);
            return ShellProbeResult::Alias { target };
        }

        // Binary: `wt is /path/to/wt`
        if let Some(pos) = line.find(" is ") {
            let after_is = &line[pos + 4..];
            if after_is.starts_with('/') || after_is.contains('\\') {
                return ShellProbeResult::Binary {
...
/// Extract alias target from a type output line.
///
/// Handles various formats:
/// - bash: `wt is aliased to `wt.exe'`
/// - zsh: `wt is an alias for wt.exe`
/// - fish: `wt is an alias to git-wt`
fn extract_alias_target(line: &str) -> String {
    // Try to find common delimiters
    // bash uses backticks: `target'
    // zsh/fish use "for" or "to" followed by the target

    // Look for backtick-quoted target (bash style)
    ...
    // Look for "to " or "for " followed by target
    for pattern in [" to ", " for "] {
        if let Some(pos) = line.find(pattern) {
            let target = line[pos + pattern.len()..].trim();
    ...
    // Fallback: return everything after "alias" if present
...
/// Probe PowerShell for command resolution.
fn probe_powershell(cmd: &str) -> ShellProbeResult {
    use std::process::{Command, Stdio};
    use std::time::{Duration, Instant};

    // PowerShell probe: Get-Command returns CommandType
    let probe_cmd = format!(
        r#"$c = Get-Command '{cmd}' -ErrorAction SilentlyContinue; if ($c) {{ Write-Output "__WT_PROBE_TYPE__$($c.CommandType)__WT_PROBE_PATH__$($c.Source)" }} else {{ Write-Output "__WT_PROBE_NOTFOUND__" }}"#
    );

    // Try pwsh (PowerShell Core) first, fall back to powershell
    let shell_bin = if cfg!(windows) {
        // On Windows, prefer pwsh but fall back to powershell
    ...
    let mut child = match Command::new(shell_bin)
        .args(["-NoProfile", "-Command", &probe_cmd])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .env_remove(crate::shell_exec::DIRECTIVE_FILE_ENV_VAR)
    ...
    let start = Instant::now();
    let timeout = Duration::from_secs(5); // PowerShell can be slow to start
    ...
    loop {
        match child.try_wait() {
            Ok(Some(_)) => {
                ...
                let stdout = String::from_utf8_lossy(&output.stdout);
                return parse_powershell_output(&stdout);
            }
            Ok(None) => {
                if start.elapsed() > timeout {
                    let _ = child.kill();
...
/// Parse PowerShell Get-Command output.
fn parse_powershell_output(output: &str) -> ShellProbeResult {
    if output.contains("__WT_PROBE_NOTFOUND__") {
        return ShellProbeResult::NotFound;
    }

    // Extract type and path from our markers
    if let Some(type_start) = output.find("__WT_PROBE_TYPE__") {
        let after_type = &output[type_start + "__WT_PROBE_TYPE__".len()..];
        if let Some(path_start) = after_type.find("__WT_PROBE_PATH__") {
            let cmd_type = after_type[..path_start].trim();
            let path = after_type[path_start + "__WT_PROBE_PATH__".len()..]
                .trim()
                .lines()
                .next()
                .unwrap_or("")
            ...
            return match cmd_type {
                "Function" => ShellProbeResult::Function,
                "Alias" => ShellProbeResult::Alias { target: path },
                "Application" | "ExternalScript" => ShellProbeResult::Binary { path },
                _ => ShellProbeResult::Binary { path },
            };
        }
    }

    ShellProbeResult::ProbeError {
        reason: "unexpected output format".to_string(),
    }
}
...
mod tests {
    ...
    // ==========================================================================
    // SHELL PROBE PARSING TESTS
    // ==========================================================================

    /// Test parsing bash/zsh `type` output for functions
    #[rstest]
    #[case::bash_function("wt is a function\nwt ()\n{", "wt", ShellProbeResult::Function)]
    #[case::zsh_function("wt is a shell function", "wt", ShellProbeResult::Function)]
    #[case::git_wt_function("git-wt is a function", "git-wt", ShellProbeResult::Function)]
    fn test_parse_bash_zsh_function(
        #[case] content: &str,
        #[case] cmd: &str,
        #[case] expected: ShellProbeResult,
    ) {
        assert_eq!(parse_bash_zsh_type(content, cmd), expected);
    }

    /// Test parsing bash/zsh `type` output for aliases
    #[rstest]
    #[case::bash_alias("wt is aliased to `wt.exe'", "wt", "wt.exe")]
    #[case::zsh_alias("wt is an alias for wt.exe", "wt", "wt.exe")]
    #[case::git_wt_alias("git-wt is aliased to `/usr/bin/git-wt'", "git-wt", "/usr/bin/git-wt")]
    fn test_parse_bash_zsh_alias(
        #[case] content: &str,
        #[case] cmd: &str,
        #[case] expected_target: &str,
    ) {
        match parse_bash_zsh_type(content, cmd) {
            ShellProbeResult::Alias { target } => assert_eq!(target, expected_target),
            other => panic!("Expected Alias, got {:?}", other),
        }
    }

    /// Test parsing bash/zsh `type` output for binaries
    #[rstest]
    #[case::unix_path("wt is /usr/local/bin/wt", "wt", "/usr/local/bin/wt")]
    #[case::windows_path(
        r"wt is C:\Program Files\wt\wt.exe",
        "wt",
        r"C:\Program Files\wt\wt.exe"
    )]
    fn test_parse_bash_zsh_binary(
        #[case] content: &str,
        #[case] cmd: &str,
        #[case] expected_path: &str,
    ) {
        match parse_bash_zsh_type(content, cmd) {
            ShellProbeResult::Binary { path } => assert_eq!(path, expected_path),
            other => panic!("Expected Binary, got {:?}", other),
        }
    }

    /// Test parsing bash/zsh `type` output for not found
    #[rstest]
    #[case::bash_not_found("bash: type: wt: not found", "wt")]
    #[case::zsh_not_found("wt not found", "wt")]
    fn test_parse_bash_zsh_not_found(#[case] content: &str, #[case] cmd: &str) {
        // Note: "not found" is handled in parse_type_output before calling parse_bash_zsh_type
        // but parse_bash_zsh_type should return NotFound if it can't parse anything
        let result = parse_type_output(content, cmd, Shell::Bash);
        assert_eq!(result, ShellProbeResult::NotFound);
    }

    /// Test parsing fish `type` output
    #[rstest]
    #[case::fish_function("wt is a function with definition", "wt", ShellProbeResult::Function)]
    #[case::fish_binary("wt is /usr/local/bin/wt", "wt", ShellProbeResult::Binary { path: "/usr/local/bin/wt".to_string() })]
    fn test_parse_fish_type(
        #[case] content: &str,
        #[case] cmd: &str,
        #[case] expected: ShellProbeResult,
    ) {
        assert_eq!(parse_fish_type(content, cmd), expected);
    }

    /// Test extract_alias_target with various formats
    #[rstest]
    #[case::bash_backtick("wt is aliased to `wt.exe'", "wt.exe")]
    #[case::zsh_for("wt is an alias for wt.exe", "wt.exe")]
    #[case::fish_to("wt is an alias to git-wt", "git-wt")]
    #[case::complex_path("wt is aliased to `/usr/local/bin/wt.exe'", "/usr/local/bin/wt.exe")]
    fn test_extract_alias_target(#[case] line: &str, #[case] expected: &str) {
        assert_eq!(extract_alias_target(line), expected);
    }

    /// Test ShellProbeResult description
    #[test]
    fn test_shell_probe_result_description() {
        assert_eq!(ShellProbeResult::Function.description(), "shell function");
        assert_eq!(
        ...
        assert_eq!(
            ...
            .description(),
            "binary at /usr/bin/wt"
        );
        assert_eq!(ShellProbeResult::NotFound.description(), "not found");
    }

    /// Test parse_type_output with markers (simulating real shell output with noise)
    #[test]
    fn test_parse_type_output_with_markers() {
        let output = r#"
Some random shell startup output
...
"#;
        assert_eq!(
            parse_type_output(output, "wt", Shell::Bash),
            ShellProbeResult::Function
        );
    }

    /// Test parse_type_output extracts correctly between markers
    #[test]
    fn test_parse_type_output_ignores_noise() {
        // Noise before and after markers should be ignored
        let output = "__WT_PROBE_START__\nwt is /usr/bin/wt\n__WT_PROBE_END__\nMore noise";
        match parse_type_output(output, "wt", Shell::Bash) {
            ShellProbeResult::Binary { path } => assert_eq!(path, "/usr/bin/wt"),
            other => panic!("Expected Binary, got {:?}", other),
        }
    }
    ...
    fn test_parse_powershell_output() {
        assert_eq!(
            parse_powershell_output("__WT_PROBE_NOTFOUND__"),
            ShellProbeResult::NotFound
        );

        assert_eq!(
            parse_powershell_output("__WT_PROBE_TYPE__Function__WT_PROBE_PATH__"),
            ShellProbeResult::Function
        );

        match parse_powershell_output("__WT_PROBE_TYPE__Application__WT_PROBE_PATH__C:\\wt.exe\n") {
            ShellProbeResult::Binary { path } => assert_eq!(path, "C:\\wt.exe"),
            other => panic!("Expected Binary, got {:?}", other),
        }
    }
