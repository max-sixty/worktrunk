...
- **Codex Cloud tools** — `scripts/codex-cloud/codex.sh` pins pre-commit, cargo-insta, cargo-nextest, Nushell, and PowerShell. Keep them level with the CI pins, since the gate there runs `--all-features` and nu and pwsh versions move PTY snapshots. `setup-web` pins Nushell and PowerShell for the same reason.
