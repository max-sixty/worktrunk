...
- **Environment tools** — `scripts/tool-pins.sh` holds every version and archive checksum for `scripts/codex-cloud/codex.sh` and the root Taskfile's `setup-web`; both source it, so one bump serves both. Keep those versions aligned with CI, and update changed archive checksums.
