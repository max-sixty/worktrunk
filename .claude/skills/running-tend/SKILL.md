...
- **Codex Cloud tools** — `scripts/codex-cloud/Taskfile.yaml` pins Task through mise and pins cargo-insta, cargo-nextest, Nushell, and PowerShell archives with SHA-256 checksums. Keep overlapping versions aligned with CI and `setup-web`, update changed archive checksums, then copy the Taskfile's new digest into both README launchers.
