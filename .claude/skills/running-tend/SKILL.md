...
- **Codex Cloud tools** — `scripts/codex-cloud/Taskfile.yaml` pins pre-commit through uv, and cargo-insta, cargo-nextest, Nushell, and PowerShell archives with SHA-256 checksums. Keep overlapping versions aligned with CI and `setup-web`, and update changed archive checksums. Task tracks latest through mise, so it needs no bump.
