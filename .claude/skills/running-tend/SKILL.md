...
Dependabot covers `Cargo.toml` dependencies and `uses: foo@vN` references in workflows, but not inline `version:` strings or action refs in composite actions. The uncovered pins drift unless this step bumps them.
...
- **`astral-sh/setup-uv` action refs** — latest from `gh api repos/astral-sh/setup-uv/releases/latest --jq '.tag_name'`. Keep `.github/actions/tend-setup/action.yaml` level with every `.github/workflows/` reference; Dependabot updates workflow references but does not scan composite actions.
