...
A cluster of these is quota exhaustion, not a bug — don't open a fix PR. Two limits produce it and they reset differently, so read the reset off the message rather than assuming the 5-hour session window; a weekly exhaustion can strand most of a day. Record the window's spend in the tracking issue, and re-run the stranded triggers only once a later run has completed cleanly — re-running into a still-exhausted quota just refills the outage issue.
