...
export def --env --wrapped {{ cmd }} [...args] {
    ...
    if (not ($args | is-empty)) and ($args | first) == "list" {
        ...
        #
        # This path still requires `sh`, unlike the exit propagation below: an
        # exec body is a POSIX snippet by construction (`ShellEscapeMode::Posix`),
        # so there is no nushell-native command to hand it to. It only fires for
        # `--execute`, and which shell should run those bodies on Windows is an
        # open question — see issue #3944.
        #
