...
- `wt list --full`, `wt list statusline` — CI status; also plain `wt list` (table) when `[list] columns` names `ci`, which forces the column (and its fetch) on without `--full`. `--format json` is exempt: it plans off `--full` alone, so a display setting can't send a machine-readable call to a forge
