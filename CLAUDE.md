...
- **Replace full files, never truncate them** — `fs::write` truncates before it writes, so a crash mid-write leaves the file empty. Every full-file rewrite worktrunk can't put back (shell wrappers, `config.toml`, `approvals.toml`, another tool's `settings.json`) goes through `utils::write_atomically`, which renames a sibling temp file over the target; the spec on that function covers symlinks, mode, and what a rename costs. Append-only updates to a user-owned file (the Bash, Zsh, and PowerShell rc files) instead open it in append mode, so a stale snapshot cannot replace concurrent edits; never truncate or rebuild such a file, and offer the addition as one buffer. Regenerable content (the cache, the `-vv` diagnostic report) keeps the plain write.
