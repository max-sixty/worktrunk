...
**A change to what a cached value means leaves the old answers in place.** The persistent entries are keyed on SHA pairs with no TTL, so the key cannot see the generator's output: an entry written by an older `wt` keeps answering until its key recurs, and on a finished branch that is never. Moving the diffs onto plumbing is the worked example — it left `has-added-changes` entries saying an unmerged branch had no added changes, which is `wt remove` deleting it. Nothing invalidates these on an upgrade, and that's the decision rather than an omission: a cache version — a stamp, or a version segment in the path — is permanent machinery for a transition that has come up once, so the remediation goes in the changelog instead. A change that alters what a cached read reports owes its changelog entry a line naming what the old answers get wrong and telling the reader that `wt config state cache clear` drops them. Ask whether you owe that line whenever a generator behind one of `sha_cache`'s kinds changes, not only for code inside `cache.rs`.
