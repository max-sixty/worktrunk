...
### Background Hook Pipelines Never Overlap in One Worktree

A hook batch splits into one detached pipeline per source, so a user-hook failure can't abort project hooks. Those pipelines run one after another: the parent spawns only the first, handing it the whole queue, and each runner spawns the next when it finishes. Running them side by side put two `git pull`s in one worktree, which is how `wt merge` came to report success with the target unpushed. A batch covers one worktree, since a `post-*` hook runs where the command leaves the user, so chaining the batch and excluding per worktree are the same thing. `wt step prune` touches several worktrees and keeps them concurrent by flushing a separate batch for each. Mechanism and the ordering guarantee it restores: the `run_pipeline` module spec.
