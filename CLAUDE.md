...
Claude Code and Codex share one payload dir, `plugins/worktrunk/`; Gemini resolves its extension at the repo root itself. Each tool hardcodes its loader path with no fallback, so the repo root carries one pointer per tool: Claude's `.claude-plugin/marketplace.json` and Codex's `.agents/plugins/marketplace.json` (both `source → ./plugins/worktrunk`), and Gemini's `gemini-extension.json` plus `hooks/hooks.json` (Gemini hard-probes `${extensionPath}/{hooks,skills}/` at the extension root, which is the repo root). Inside `plugins/worktrunk/`: `plugin.json` is the Claude manifest (at the plugin root — Claude's `.claude-plugin/` wrapper is marketplace-root-only) and `.codex-plugin/plugin.json` is the Codex manifest (Codex's required wrapper). The repo-root `skills/` is the single source — Claude/Codex reach it via `plugins/worktrunk/skills -> ../../skills`, Gemini via `${extensionPath}/skills` (the root) — and it feeds the docs auto-sync. The full layout, path-resolution rules, and live-CLI verification are documented in `plugins/worktrunk/CLAUDE.md`; keep that file current when the layout changes.

**Gemini extension — repo root.** Gemini hard-probes `${extensionPath}/hooks/hooks.json` and `${extensionPath}/skills/` with no manifest path indirection, and `gemini extensions install owner/repo` reads `gemini-extension.json` only at the clone root. So Gemini's manifest is itself a loader-mandated root pointer (the third tool pointer), `hooks/hooks.json` sits at the repo root, and `${extensionPath}/skills/` resolves to the real single-sourced repo-root `skills/`. The hooks call the canonical `${extensionPath}/plugins/worktrunk/hooks/wt.sh` by its real path — no symlink, no bundled copy, no drift guard. Native `gemini extensions install owner/repo` works (`gemini extensions validate .` is the non-interactive check). This replaced an earlier `plugins/gemini/` sibling dir whose `skills` symlink and bundled `wt.sh` existed only to survive `gemini extensions install`'s `fs.cp`, and which broke github-URL install entirely; relocating to the root deletes all of that. `test_plugin_layout_is_consolidated` guards the layout.
...
**Accepted tradeoff — the shared `skills/` exposes `wt-switch-create` to Codex and Gemini.** Codex's `"skills": "./skills/"` and Gemini's `${extensionPath}/skills/` both resolve the entire repo-root `skills/` directory — including `wt-switch-create`, which depends on Claude session-cwd switching and the `WorktreeCreate` hook neither provides. We accept this: a tool loading a skill it can't act on is harmless, and a single repo-root `skills/` keeps the `worktrunk` skill single-source across all three tools and the docs sync. Don't add per-tool skills subtrees to exclude it.
